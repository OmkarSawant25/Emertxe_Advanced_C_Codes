/*
Name        : Omkar Ashok Sawant
Date        : 07/10/2025
Description : WAP to get 'n' bits from given position of a number
              a. Read number num from user.
              b. Read number n from user.
              c. Read number pos from user.
              d. Fetch n number of bits from given position 'pos' (starting from LSB) of num and return the decimal value of it.
              If num is 12, n is 3 and pos is 4
                    7 6 5 4 3 2 1 0
              12 -> 0 0 0 0 1 1 0 0
              The program should print 3 (0 1 1).
I/O         : I  -> Enter the number: 12
                    Enter number of bits: 3
                    Enter the pos: 4
              O  -> Result = 3
*/

#include <stdio.h>

int get_nbits_from_pos(int, int, int);

int main()
{
    int num, n, pos, res = 0;

    printf("Enter num, n and val:"); // Ask the user to enter the number, how many bits to take, and from which position
    scanf("%d%d%d", &num, &n, &pos);

    res = get_nbits_from_pos(num, n, pos);

    printf("Result = %d\n", res);
}

int get_nbits_from_pos(int num, int n, int pos)
{
    num = num >> pos - n + 1;   // Move the required bits to the LSB side
    num = num & ((1 << n) - 1); // Create a mask with 'n' ones
    return num;
}